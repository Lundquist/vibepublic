...
import { setCurrentCompany } from '../global'
...
export const GET_USER_COMPANIES = '[USER] GET COMPANY';
...
export function setUserData(user) {
    return (dispatch) => {
        ...
        Set User Settings
         */
        dispatch(getUserCompanies(user.data.email));
        dispatch(setDefaultSettings(user.data.settings));

        /*
        Set User Data
         */
        console.log("setUserData " + JSON.stringify(user))
        dispatch({
            type: SET_USER_DATA,
            payload: user
        })
 ...
 * 
 * @param {*} user
 * Get all companies that has this employee 
 */
export function getUserCompanies(email) {
    const request = axios.get(`${config.serverUrl}/getCompaniesForEmployee?employeeEmail=${email}`, { 
    ...
    return (dispatch) => {
         request.then((response) => {
            if (response.data.Rows.length !== 0) {
                ...
                dispatch(setCurrentCompany(response.data.Rows[0].id));
            } else {
                Actions.createNewCompany(
                    {
                        role: 'admin',
                        email: email
                    }
